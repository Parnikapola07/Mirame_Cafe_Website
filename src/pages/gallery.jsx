      ...
      <section className="py-24 bg-cream">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {images.map((img, idx) => (
              <div key={idx} className="break-inside-avoid shadow-premium rounded-lg overflow-hidden group cursor-pointer relative">
                {/* Image Zoom */}
                <img
                  src={img.src}
                  alt={img.alt}
                ...
                {/* Espresso Overlay */}
                <div className="absolute inset-0 bg-espresso/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0">
                  {/* Magnifying Glass Scale & Rotate */}
                  <i className="fa-solid fa-magnifying-glass-plus text-3xl text-gold transform scale-75 group-hover:scale-110 rotate-[-45deg] group-hover:rotate-0 transition-all duration-500"></i>
                </div>

                {/* Gold Inner Border Overlay */}
                <div className="absolute inset-4 border border-gold/40 opacity-0 group-hover:opacity-100 transition-all duration-500 scale-95 group-hover:scale-100 pointer-events-none z-10"></div>
              </div>
            ))}
          </div>
        </div>
